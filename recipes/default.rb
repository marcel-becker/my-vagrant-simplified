...
script "update_apt_ppa_repos" do
    cwd Chef::Config[:file_cache_path]
   interpreter "bash"
   user "root"
   code <<-EOH
# gnome themes
#apt-add-repository -y  ppa:upubuntu-com/gtk3
#apt-add-repository -y  ppa:upubuntu-com/gtk+3.6
# xfce theme manager
apt-add-repository -y  ppa:rebuntu16/other-stuff
# ubunt app indicator
apt-add-repository -y  ppa:atareao/atareao
# sublime text editor
apt-add-repository -y  ppa:webupd8team/sublime-text-2
# x2go remote desktop manager
...
apt-get update
   EOH
   only_if do ! File.exists?("/etc/apt/sources.list.d/x2go-repo.list") end
end
...
script "install_apt_get-packages" do
    cwd Chef::Config[:file_cache_path]
   interpreter "bash"
   user "root"
   code <<-EOH
apt-get install -y build-essential synaptic make cmake libgtk2.0-dev gtk-chtheme compizconfig-settings-manager compiz-plugins-extra terminator
apt-get install -y linux-headers-generic ipython python-software-properties msgpack-python
apt-get install -y scapy python-twisted python-boto python-zodb bridge-utils
apt-get install -y elementary zukitwo zukiwi elegant-brit-theme gnomishgray drakfire-zuki
apt-get install -y delorean-noir-theme ice-cream-theme light-greyness-dark-grey ice-redux oceanic-dark
apt-get install -y mediterranean-night-themes thewidgetfactory calendar-indicator sublime-text
apt-get install -y xubuntu-desktop xfce4 xfce-keyboard-shortcuts
apt-get install -y xfce-theme-manager xfwm4-composite-editor xfwm4-themes
apt-get install -y gnome-tweak-tool
apt-get install -y libsnappy-dev python-pip libssl-dev ansible python-apt virtualenv
apt-get install -y x2goserver x2goserver-xsession x2goclient
...
DEBIAN_FRONTEND=noninteractive apt-get build-dep emacs24 -y
cd emacs-24.5
./configure
make
make install
...
# script "change_file_ownership" do
#    interpreter "bash"
#    user "root"
#    cwd "/home/becker/"
#    code <<-EOH
